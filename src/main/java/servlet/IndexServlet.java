...
public class IndexServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 词频统计
        Pair<String,Integer>[] maps = tf_idf.getResult(words,stockInfos);

        String[] arrays = new String[maps.length];
        int i = 0, inputMin = maps[maps.length - 1].getValue(), inputMax = maps[0].getValue(), outputMin = 10, outputMax = 120;

        for (Pair<String,Integer> s : maps) {
            //生成词云关键字
     ...
     * @return 变化后
     */
    private int linerScale(int inputMin,int inputMax,int outputMin,int outputMax,int item){
        float var1 = (outputMax - outputMin + 0.0f) / (inputMax - inputMin + 0.0f);
        float var2 = outputMax - var1 * inputMax;
